...
export function lonlat2xyz (coord) {
  const lon = coord[0] * RADIANS
  const lat = coord[1] * RADIANS
...
export function euler2quat (e) {
  ...
  const roll = 0.5 * e[0] * RADIANS
  const pitch = 0.5 * e[1] * RADIANS
  const yaw = 0.5 * e[2] * RADIANS
...
export function quat2euler (t) {
  ...
  return [
    Math.atan2(2 * (t[0] * t[1] + t[2] * t[3]), 1 - 2 * (t[1] * t[1] + t[2] * t[2])) * DEGREES,
    Math.asin(Math.max(-1, Math.min(1, 2 * (t[0] * t[2] - t[3] * t[1])))) * DEGREES,
    Math.atan2(2 * (t[0] * t[3] + t[1] * t[2]), 1 - 2 * (t[2] * t[2] + t[3] * t[3])) * DEGREES
  ]
}
...
export const eulerAngles = (v0, v1, o0) => quat2euler(
  quatMultiply(
    euler2quat(o0),
    quaternion(
      lonlat2xyz(v0),
